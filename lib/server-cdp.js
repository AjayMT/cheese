...
CDP.ioHandler = function (io, socket, Cheese) {
  ...
  if (Cheese.filters.all)
    diff = CDP.applyFilters(Cheese.filters.all, diff);

  if (Cheese.filters.each)
    diff = CDP.applyFilters(Cheese.filters.each, diff, {});

  CDP.clients.push(diff);
  ...
  socket.on('custom', function (msg) {
    ...
    if (Cheese.messageHandlers[msg.msg]) {
      Cheese.messageHandlers[msg.msg](msg.args, sock,
  ...
  socket.on('msg', function (diff) {
    ...
    if (! CDP.allowDiff(Cheese.allows,
      ...
      var clientDB = diffUtils.copyObject(CDP.clients[index]);
      diffUtils.applyDiff(diff, clientDB);
      var newDiff = diffUtils.createDiff(clientDB, CDP.clients[index]);
