...
function applyFilters (filters, diff, client) {
  for (var f in filters)
    diff = filters[f].func(diffUtils.copyObject(diff), client);
...
function separateFilters (filters) {
  ...
  for (var f in filters) {
    if (filters[f].each) separated.each.push(filters[f]);
    else separated.all.push(filters[f]);
  }
...
module.exports.ioHandler = function (io, socket, Cheese) {
  ...
  var filters = separateFilters(Cheese.filters);
  var diff = diffUtils.copyObject(Cheese.db);

  if (filters.all)
    diff = applyFilters(filters.all, diff);

  if (filters.each)
    diff = applyFilters(filters.each, diff, {});
  ...
  if (Cheese.connectHandler) {
    ...
    filters = separateFilters(Cheese.filters);
  ...
  function updateServer (diff) {
    ...
    if (filters.all && (! filters.each))
      socket.broadcast.emit('msg', applyFilters(filters.all,
                                                diffUtils.copyObject(diff)));
    else if (filters.each)
      for (var i in clients) {
        ...
        if (filters.all)
          finalDiff = applyFilters(filters.all,
                                   diffUtils.copyObject(finalDiff));

        sockets[i].emit('msg', applyFilters(filters.each,
                                            diffUtils.copyObject(finalDiff),
                                            clients[i]));
      }
    ...
    for (var j in clients) {
      diffUtils.applyDiff(clients[j], applyFilters(filters.all,
                                                   diffUtils.copyObject(diff)));
      diffUtils.applyDiff(clients[j], applyFilters(filters.each,
                                                   diffUtils.copyObject(diff),
                                                   clients[j]));
    }
  }
  ...
  function updateClients (client) {
    ...
    if (filters.all && (! filters.each))
      io.emit('msg', applyFilters(filters.all, diffUtils.copyObject(diff)));
    else if (filters.each)
      for (var i in clients) {
        diff = diffUtils.createDiff(clients[i], Cheese.db);

        if (filters.all)
          diff = applyFilters(filters.all, diffUtils.copyObject(diff));
