...
function serverHandler (req, res) {
  ...
  if (req.url === '/__client/client.js') {
    ...
    if (content === undefined) {
      if (staticData[requestPath]) content = staticData[requestPath];
      else content = staticData[requestPath.split('/').slice(1).join('/')]
    }
...
function start (portArg, clientDataArg, staticDataArg, mainFilePathOrDone, done) {
  ...
  if (Cheese.opts.ssl)
    server = https.createServer(serverHandler, { key: Cheese.opts.ssl.key, cert: Cheese.opts.ssl.cert });
  else
...
function reload (clientDataArg, staticDataArg, mainFilePath) {
  clientData = clientDataArg;
  staticData = staticDataArg;
