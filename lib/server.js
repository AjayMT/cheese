...
server.res = {
  diffScript: fs.readFileSync(path.join(__dirname, '..',
                                        'node_modules',
                                        'cdp-diff', 'lib', 'cdp-diff.js'),
                              { 'encoding': 'utf-8' }) + '\n',
  DOMScript: fs.readFileSync(path.join(__dirname, '..',
                                       'node_modules',
                                       'ezdom', 'lib', 'ezdom.js'),
                             { 'encoding': 'utf-8' }) + '\n',
  clientScript: fs.readFileSync(path.join(__dirname, 'client.js'),
                                { 'encoding': 'utf-8' }) + '\n'
}

var jQueryPath = path.join(__dirname, '..', 'node_modules',
...
server.serverHandler = function (req, res) {
  ...
  if (req.url === '/__client/client.js') {
    ...
    res.end(jQueryScript + server.res.diffScript
           + server.res.DOMScript
           + server.res.clientScript
           + server.clientData);
